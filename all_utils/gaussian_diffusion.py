...
from all_utils.nn import mean_flat
from all_utils.losses import normal_kl, discretized_gaussian_log_likelihood, discretized_text_log_likelihood
...
class GaussianDiffusion:
    ...
    def training_losses_e2e(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.E2E_KL:
            ...
            # print(x_start.shape,model_output.shape, target.shape)
            assert model_output.shape == target.shape == x_start.shape
            terms["mse"] = mean_flat((target - model_output) ** 2)
            # print( terms["mse"])
