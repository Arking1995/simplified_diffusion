...
All arguments now in parser.py now. Basically what we need to change across different dataset is 
    --batch_size  "defaultly 64, it only uses about 8GB memory on my local GPU"
    --lr           "learning rate"
    --lr_anneal_steps   "how many steps you wanna train"
    --diffusion_steps  "how many diffusion steps are used in this training"
...
For better debugging, model initilization in "utils.py", trainer in "train_utils.py", dataset loading in "text_dataset.py", models are in "gaussian_diffusion.py", 
